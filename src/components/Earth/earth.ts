...
const EARTH_COLOR = 0x624bc8;
const EARTH_PARTICLE_COLOR = 0x576c73;
...
export class Earth {
  ...
  constructor(radius: number) {
    ...
    const material = new THREE.MeshBasicMaterial({ color: 0x624bc8, side: THREE.DoubleSide });
  ...
  private createEarthParticles() {
    ...
    for (let i = 0; i < step; i++) {
      ...
      for (let j = 0; j < step; j += radians) {
        ...
        if (isLandByUV(c, f, { earthImgData: this.earthImgData, width: this.earthImg.width, height: this.earthImg.height })) {
          // 根据横纵百分比判断在底图中的像素值
